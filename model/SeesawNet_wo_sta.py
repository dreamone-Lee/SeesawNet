...
from layers.SeesawNet_wo_sta_backbone import TSEncoder, PD_Attention_layer, CR_Attention_layer, aggregation_layer
...
class Model(nn.Module):
    ...
    def layers_init(self, configs):
        PD_Attention = [PD_Attention_layer(
            self.patch_num, configs.d_model, configs.d_ff, configs.n_heads, dropout=configs.dropout, 
        ...
        return [*PD_Attention, Aggregation, *CR_Attention]
    ...
    def forward(self, x, x_mark=None):
        ...
        enc_out = self.encoder(z, z)[0][:, :self.c_in, ...]
        dec_out = self.decoder(enc_out).transpose(-1, -2)

        return dec_out[:, -self.pred_len:, :], None, None  # [B, L_pred, N]
